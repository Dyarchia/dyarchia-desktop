...
from dyarchia_crawlee.models import PageStatus, utcnow
...
class RunManifest(BaseModel):
    ...
    `version` and `created_at` are written and never read back, and they are kept deliberately.
    A format version nothing consults yet is exactly what lets an old manifest be recognised the
    day the format moves, and the timestamp answers "when was this corpus last written" for whoever
    opens the file. Neither is the currency check: that one compares the change report against the
    sweep that covered it, in `digest`.
    """
