...
from euripontida_crawlee import __version__, digest, inventory, locking, registry, state
from euripontida_crawlee.config import Settings, get_settings
from euripontida_crawlee.engine import RunResult, execute
from euripontida_crawlee.errors import ConfigurationError, CrawleeLabError
from euripontida_crawlee.models import CrawlerKind, ExtractionMode, LinkStrategy, OutputFormat, RunSpec
from euripontida_crawlee.profiles import loader as profile_loader
from euripontida_crawlee.profiles.schema import ProfileSpec
from euripontida_crawlee.recon import Recon, inspect_url
from euripontida_crawlee.storage.exporters import slugify_url
from euripontida_crawlee.storage.snapshots import SnapshotResult
from euripontida_crawlee.versioning.diffing import ChangeKind, load_report
from euripontida_crawlee.versioning.report import summary_line
...
def diff_command(
    ...
    if report is None:
        error_console.print(
            ...
            f'Run "euripontida-crawlee crawl --profile {name} --snapshot" first.[/bold red]'
        )
        raise typer.Exit(code=1)
...
@app.command()
def version() -> None:
    """Print the euripontida-crawlee version."""
