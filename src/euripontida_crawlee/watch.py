...
from euripontida_crawlee.engine import execute
from euripontida_crawlee.errors import CrawleeLabError
from euripontida_crawlee.models import utcnow
from euripontida_crawlee.versioning.diffing import CHANGES_FILENAME
...
def render_markdown(result: WatchResult) -> str:
    ...
    for entry in result.entries:
        ...
        if entry.failed:
            ...
            if entry.changed:
                lines.append('')
                lines.append(f'See `euripontida-crawlee diff {entry.name} --unified`.')
        lines.append('')
